...
int x_axis_offset = 266;
int y_axis_offset = 249;

//Joystick direction offset per axis
int x_axis_right_offset = 245;
int x_axis_left_offset = 265;
int y_axis_forward_offset = 249;
int y_axis_backwards_offset = 262;
...
// Checks if mpu6050 has been turned on
  ...
  // Going forward
    ...
    //Going Backwards
    if (y_axis > 249){
      y_axis = map(y_axis-y_axis_offset, 0, y_axis_backwards_offset, 0, motor_maxspeed);
      motorcontrols[0] = y_axis;
      motorcontrols[2] = 2;
      
  // Going Forward
  } else if (y_axis <= 249) {
      y_axis = map((y_axis-y_axis_offset)*-1, 0, y_axis_forward_offset, 0, motor_maxspeed);
      motorcontrols[0] = y_axis;
      motorcontrols[2] = 1; 
    } 
    ...
    // Right
    if (x_axis >= 266){
      x_axis = map(x_axis-x_axis_offset, 0, x_axis_right_offset, 0, motor_maxspeed);
      motorcontrols[1] = x_axis;
      motorcontrols[3] = 1;
      
    // Left
    } else if (x_axis < 266) {
      x_axis = map((x_axis-x_axis_offset)*-1, 0, x_axis_left_offset, 0, motor_maxspeed);  
      motorcontrols[1] = x_axis;
      motorcontrols[3] = 2;
    }
  Serial.print("\n");
